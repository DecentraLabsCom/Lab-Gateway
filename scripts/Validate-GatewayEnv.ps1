...
$gatewayId = ([string]$values['SERVER_NAME']).Trim().TrimEnd('.').ToLowerInvariant()
...
if (-not $redeemers.ContainsKey($gatewayId) -or $redeemers[$gatewayId] -cne $redeemer) {
    throw 'ACCESS_CODE_REDEEMER_CREDENTIALS_JSON must contain the Full gateway SERVER_NAME mapped to AUTH_ACCESS_CODE_REDEEMER_TOKEN'
}

$observerId = ([string]$values['SESSION_OBSERVER_GATEWAY_ID']).Trim().TrimEnd('.').ToLowerInvariant()
if ($observerId -cne $gatewayId) {
    throw 'SESSION_OBSERVER_GATEWAY_ID must match SERVER_NAME in Full mode'
}
...
if (-not $observers.ContainsKey($gatewayId) -or $observers[$gatewayId] -cne $observerSecret) {
    throw 'SESSION_OBSERVER_CREDENTIALS_JSON must contain the Full gateway SERVER_NAME mapped to SESSION_OBSERVER_SIGNING_SECRET'
}
