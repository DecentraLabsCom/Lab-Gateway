...
| Public lab-status projection | `LAB_STATUS_HEARTBEAT_MAX_AGE_SECONDS` | Maximum age of the persisted Lab Station heartbeat used by `GET /public/labs/status`; older or missing signals are returned as `unknown`, never as `not_ready`. |
