...
per-lab projection. It returns only the state, signal age and stable reason for
each requested lab. A fresh Lab Station heartbeat is authoritative; when no
fresh heartbeat exists, the Ops Worker may probe the configured Guacamole RDP,
