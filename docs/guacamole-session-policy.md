...
Guacamole. Only then does it commit a session observation to its local MySQL
outbox. The worker retries delivery with backoff and marks it sent only after the
backend confirms both audit and signed-attestation persistence (`recorded=true`).
Rejected upgrades, invalid tokens and unavailable remote desktops therefore do
not create `SessionStarted`. Manual and administrator sessions keep their
existing operational behavior and are not reservation observations.
