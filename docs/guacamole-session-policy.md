...
The policy is local to each access gateway. In Full mode the backend service
...
Browser hand-off uses an opaque access code with a short-lived redemption lease. OpenResty reserves the code, validates the signed lab-access JWT locally, and commits it only after the local destination and session checks pass; a failed check releases the lease. The JWT is never placed in the Guacamole URL; the browser receives only the Secure, HttpOnly JTI cookie.
...
`SESSION_OBSERVER_SIGNING_SECRET` from a Full-issued trust bundle. The observer
ID is the lower-case public host plus a non-default HTTPS port, matching the
`targetGatewayId` derived from `accessURI`. Outbox delivery
