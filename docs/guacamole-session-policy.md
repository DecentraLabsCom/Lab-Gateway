...
For reservation users, OpenResty persists the session-start observation synchronously during the WebSocket request's access phase, before proxying the upgrade. Ops Worker must commit the local MySQL outbox record before acknowledging; otherwise OpenResty rejects the request with `503`. The worker then retries delivery with backoff and marks it sent only after the backend confirms both audit and signed attestation persistence (`recorded=true`). Manual and administrator Guacamole sessions are not reservation observations and keep their existing operational behavior.
