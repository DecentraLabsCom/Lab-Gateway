...
overlay is required when Docker bridge traffic is not forwarded to the LAN; it
leaves the existing private Compose networks in place. Configure the Station
subnet, parent NIC, reserved container range, and optional VLAN gateway. Leave
the gateway empty for a directly connected isolated subnet. Configure the
directed broadcast per host as described in the [Gateway configuration
