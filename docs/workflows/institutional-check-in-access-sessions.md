...
Before OpenResty exposes a Guacamole token to the browser, Ops Worker encrypts it and inserts `guacamole_token_revocation_queue`; a failed durable insert therefore fails the login closed. Ops Worker reconciles active connections and the durable Guacamole connection history using the exact encrypted token. Historical evidence remains eligible for the configurable `GUACAMOLE_HISTORY_RECONCILIATION_RETENTION_SECONDS` window after `expires_at`, while revocation still occurs at expiry and authorization is never extended. Terminal revocation failures are reported through the degraded health response.
