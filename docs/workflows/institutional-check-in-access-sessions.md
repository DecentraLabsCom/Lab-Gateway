...
Before OpenResty exposes a Guacamole token to the browser, Ops Worker encrypts it and inserts `guacamole_token_revocation_queue`; a failed durable insert therefore fails the login closed. Registration records the token's pre-revocation validation. Before revocation, active observations still require the exact token to be accepted. After revocation, historical evidence is authorized by that marker plus the unique temporary username, reservation key, observer JTI and issuance/expiry window matched against `guacamole_connection_history`; the revoked bearer is never reused. Historical evidence remains eligible for the configurable `GUACAMOLE_HISTORY_RECONCILIATION_RETENTION_SECONDS` window after `expires_at`, while revocation still occurs at expiry and authorization is never extended. Terminal revocation failures are reported through the degraded health response.
