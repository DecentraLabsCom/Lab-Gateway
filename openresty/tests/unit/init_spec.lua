...
runner.describe("OpenResty init.lua", function()
    ...
    runner.it("stores custom JWT Guacamole idle timeout", function()
        local ngx = run_init({
            env = {
                ...
                SERVER_NAME = "gateway.example",
                HTTPS_PORT = "443",
                JWT_GUAC_IDLE_TIMEOUT_SECONDS = "300"
            },
            files = {
    ...
    runner.it("derives token security retention from the Guacamole API timeout", function()
        local ngx = run_init({
            env = {
                GUAC_ADMIN_USER = "admin",
                GUAC_ADMIN_PASS = "really-strong-secret",
        ...
        runner.assert.equals(2100, ngx.shared.config:get("guac_token_security_retention_seconds"))
    end)
