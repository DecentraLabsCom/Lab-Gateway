...
runner.describe("Lab manager access guard", function()
    ...
    runner.it("keeps the Ops credential after the /ops path is internally rewritten", function()
        local ngx = run_lab_manager_access({
            env = {
                LAB_MANAGER_TOKEN = "browser-token",
                OPS_INTERNAL_AUTH_TOKEN = "ops-token",
            ...
            var = {
                ...
                uri = "/api/operations/recent",
                request_uri = "/ops/api/operations/recent?limit=50"
            }
        })
